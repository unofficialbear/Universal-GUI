...
do -- Example UI
	local Window = library:AddWindow("ProtoSmasher v2", {
		main_color = Color3.fromRGB(200, 0, 0),
		min_size = Vector2.new(700, 400),
		toggle_key = Enum.KeyCode.LeftAlt,
		can_resize = true,
	})
	local execution = Window:AddTab("Execution")
    local scrip = Window:AddTab("Scripts")
    local Output = Window:AddTab("Output")
    local Settings = Window:AddTab("Settings")
    local ScriptViewer = Window:AddTab("Script Viewer")
    local Console = Window:AddTab("Console")
    local Theme = Window:AddTab("Theme")
    
    
	do -- Elements
		local Console = execution:AddConsole({
			["y"] = 305,
			["source"] = "Lua",
		})
		Console:Set('print("Hello, world!")')
		local pro = execution:AddHorizontalAlignment()
        local exec = pro:AddButton("Execute", function()
            loadstring(Console:Get())()
        end)
        local clr = pro:AddButton("Clear", function()
            Console:Set('')
        end)
        local of = pro:AddButton("Open")
        local save = pro:AddButton("Save")
        local ef = pro:AddButton("Execute file")
	end
